module vector_alu_tb;
    ...
    for (genvar lane = 0; lane < (LANE_SIZE); lane++) begin : connect_dut_lane_out
        assign lane_out[lane] = dut_out_ch.data[lane*(8)+:(8)];
    end
    assign out_valid = dut_out_ch.valid;
    assign dut_out_ch.ready = out_ready;
    vector_alu #(
        .LANE_SIZE(LANE_SIZE)
    ) dut (
        .clk(clk),
    ...
    always @(posedge clk) begin
        ...
        if (!rst_n) begin
            ...
            if (stalled && (!out_valid || actual !== held))
    ...
    initial begin
        ...
        for (int a = 0; a < 256; a++) for (int b = 0; b < 256; b++) send(op, a, b, b % 2, 0);
        ...
        for (int n = 0; n < 5000; n++) begin
            send(int'($urandom_range(0, 31)), int'($urandom_range(0, 255)), int'($urandom_range(
                 0, 255)), int'($urandom_range(0, 1)), 1);
            if (n % 17 == 0) begin
                valid = 0;
                repeat (3) @(negedge clk);
            end
        ...
        for (int op = 0; op < 32; op++) if (op_count[op] == 0) $fatal(1, "opcode not covered");
        ...
        $display(
            "PASS ALU lanes=%0d accepted=%0d consumed=%0d reset_flushed=%0d full=%0d simultaneous=%0d max_run=%0d",
