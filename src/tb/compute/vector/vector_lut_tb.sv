module vector_lut_tb;
    ...
    assign core_dut_in_ch.addr = '0;
    for (genvar lane = 0; lane < (LANE_SIZE); lane++) begin : connect_core_dut_lane_in_a
        assign core_dut_in_ch.data[lane*(8)+:(8)] = data_in[lane];
    end
    for (genvar lane = 0; lane < (LANE_SIZE); lane++) begin : connect_core_dut_lane_in_b
        assign core_dut_in_ch.data[(8)*(LANE_SIZE)+lane*(8)+:(8)] = lane_b[lane];
    end
    assign core_dut_in_ch.data[2*(8)*(LANE_SIZE)] = lane_sel;
    assign core_dut_in_ch.data[2*(8)*(LANE_SIZE)+1+:5] = opcode;
    assign core_dut_in_ch.valid = valid;
    assign ready = core_dut_in_ch.ready;
    rv_if #(
        .ADDR_WIDTH(1),
        .DATA_WIDTH((8) * (LANE_SIZE))
    ) core_dut_out_ch ();
    for (genvar lane = 0; lane < (LANE_SIZE); lane++) begin : connect_core_dut_lane_out
        assign lane_out[lane] = core_dut_out_ch.data[lane*(8)+:(8)];
    end
    assign out_valid = core_dut_out_ch.valid;
    assign core_dut_out_ch.ready = out_ready;
    vector_alu #(
        .LANE_SIZE(LANE_SIZE)
    ) core_dut (
        .clk(clk),
        .rst_n(rst_n),
        .in_ch(core_dut_in_ch),
        .out_ch(core_dut_out_ch)
    );

    initial begin
        // Every lane visits all 256 bit patterns, with different inputs per lane.
        for (int pattern = 0; pattern < 256; pattern++) begin
            for (int lane = 0; lane < LANE_SIZE; lane++) data_in[lane] = 8'(pattern + 17 * lane);
            #1;
            for (int lane = 0; lane < LANE_SIZE; lane++) begin
                ...
                if (int'(exp_out[lane]) != expected_exp)
                    $fatal(
                        1,
                        "exp lane=%0d x=%0d got=%0d expected=%0d",
                        lane,
                        x,
                        exp_out[lane],
                        expected_exp
