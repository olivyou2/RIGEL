...
SOURCES=("$ROOT/src/interfaces/rv_if.sv" "$ROOT/src/compute/vector/vector_core/vector_exp.sv" "$ROOT/src/compute/vector/vector_core/vector_sqrt.sv")
# Compile the ALU and its LUT dependencies explicitly.
verilator --lint-only --timing -Wno-TIMESCALEMOD --top-module vector_lut_tb \
    "${SOURCES[@]}" "$ROOT/src/compute/vector/vector_core/vector_alu.sv" "$ROOT/src/tb/compute/vector/vector_lut_tb.sv"
