module dma_addr_tb ();
    ...
    assign dma_addr_in_ch.data = data_in;
    assign dma_addr_in_ch.valid = data_in_valid;
    assign data_in_ready = dma_addr_in_ch.ready;
    assign dma_addr_in_ch.addr = '0;
    rv_if #(
        .ADDR_WIDTH((ADDR_WIDTH)),
        .DATA_WIDTH((DATA_WIDTH))
    ) dma_addr_out_ch ();
    assign addr_out = dma_addr_out_ch.addr;
    assign data_out = dma_addr_out_ch.data;
    assign data_out_valid = dma_addr_out_ch.valid;
    assign dma_addr_out_ch.ready = data_out_ready;
    dma_dst_addr #(
        ...
        .in_ch(dma_addr_in_ch),
        .out_ch(dma_addr_out_ch)
    );

    task automatic set_dst_addr(input logic [ADDR_WIDTH-1:0] addr_dst,
