module mesh_router_tb ();
    ...
    assign dut_north_ch.addr  = north_addr_in;
    assign dut_north_ch.data  = north_data_in;
    assign dut_north_ch.valid = north_data_valid;
    assign north_data_ready   = dut_north_ch.ready;
    rv_if #(
        .ADDR_WIDTH((ADDR_WIDTH)),
        .DATA_WIDTH((DATA_WIDTH))
    ) dut_west_ch ();
    assign dut_west_ch.addr  = west_addr_in;
    assign dut_west_ch.data  = west_data_in;
    assign dut_west_ch.valid = west_data_valid;
    assign west_data_ready   = dut_west_ch.ready;
    rv_if #(
        .ADDR_WIDTH((ADDR_WIDTH)),
        .DATA_WIDTH((DATA_WIDTH))
    ) dut_east_ch ();
    assign east_addr_out = dut_east_ch.addr;
    assign east_data_out = dut_east_ch.data;
    assign east_data_valid = dut_east_ch.valid;
    assign dut_east_ch.ready = east_data_ready;
    rv_if #(
        .ADDR_WIDTH((ADDR_WIDTH)),
        .DATA_WIDTH((DATA_WIDTH))
    ) dut_south_ch ();
    assign south_addr_out = dut_south_ch.addr;
    assign south_data_out = dut_south_ch.data;
    assign south_data_valid = dut_south_ch.valid;
    assign dut_south_ch.ready = south_data_ready;
    rv_if #(
        .ADDR_WIDTH((ADDR_WIDTH)),
        .DATA_WIDTH((DATA_WIDTH))
    ) dut_local_in_ch ();
    assign dut_local_in_ch.addr  = local_addr_in;
    assign dut_local_in_ch.data  = local_data_in;
    assign dut_local_in_ch.valid = local_data_in_valid;
    assign local_data_in_ready   = dut_local_in_ch.ready;
    rv_if #(
        .ADDR_WIDTH((ADDR_WIDTH)),
        .DATA_WIDTH((DATA_WIDTH))
    ) dut_local_out_ch ();
    assign local_addr_out = dut_local_out_ch.addr;
    assign local_data_out = dut_local_out_ch.data;
    assign local_data_out_valid = dut_local_out_ch.valid;
    assign dut_local_out_ch.ready = local_data_out_ready;
    mesh_router #(
        .ADDR_WIDTH(ADDR_WIDTH),
        .DATA_WIDTH(DATA_WIDTH),
        .X_BITS    (X_BITS),
        ...
        .clk(clk),
        .rst_n(rst_n),
        .north_ch(dut_north_ch),
        .west_ch(dut_west_ch),
        .east_ch(dut_east_ch),
        .south_ch(dut_south_ch),
        .local_in_ch(dut_local_in_ch),
        .local_out_ch(dut_local_out_ch)
    );
    ...
    function automatic logic [ADDR_WIDTH-1:0] make_addr(
        input logic [X_BITS-1:0] x, input logic [Y_BITS-1:0] y,
    ...
    task automatic send_packet(input int source, input logic [ADDR_WIDTH-1:0] addr,
    ...
    task automatic expect_packet(input int destination, input logic [ADDR_WIDTH-1:0] expected_addr,
        ...
        case (destination)
            EAST: begin
                ...
                if (east_addr_out !== expected_addr || east_data_out !== expected_data)
                    $fatal(
                        1,
                        "EAST mismatch: addr=%h data=%h, expected addr=%h data=%h",
                        east_addr_out,
                        east_data_out,
                        expected_addr,
                        expected_data
            ...
            SOUTH: begin
                ...
                if (south_addr_out !== expected_addr || south_data_out !== expected_data)
                    $fatal(
                        1,
                        "SOUTH mismatch: addr=%h data=%h, expected addr=%h data=%h",
                        south_addr_out,
                        south_data_out,
                        expected_addr,
                        expected_data
            ...
            LOCAL: begin
                ...
                if (local_addr_out !== expected_addr || local_data_out !== expected_data)
                    $fatal(
                        1,
                        "LOCAL mismatch: addr=%h data=%h, expected addr=%h data=%h",
                        local_addr_out,
                        local_data_out,
                        expected_addr,
                        expected_data
    ...
    task automatic check_route(input int source, input int destination,
                               input logic [ADDR_WIDTH-1:0] addr,
                               input logic [DATA_WIDTH-1:0] data);
        $display("[MESH_ROUTER_TB] route source=%0d destination=%0d data=%h", source, destination,
                 data);
        fork
    ...
    always @(posedge clk) begin
        if (collect_east && east_data_valid && east_data_ready) begin
            if (east_count >= 3) $fatal(1, "EAST produced more packets than expected");
    ...
    task automatic check_backpressure;
        ...
        @(posedge clk);
        @(negedge clk);
        if (east_data_valid !== 1'b0) $fatal(1, "EAST valid did not clear after handshake");
    ...
    initial begin
        ...
        check_route(NORTH, LOCAL, make_addr(ROUTER_X, ROUTER_Y, 28'h0000013),
        ...
        check_route(WEST, LOCAL, make_addr(ROUTER_X, ROUTER_Y, 28'h0000023),
        ...
        check_route(LOCAL, LOCAL, make_addr(ROUTER_X, ROUTER_Y, 28'h0000033),
                    64'h0000_0000_0000_0033);
